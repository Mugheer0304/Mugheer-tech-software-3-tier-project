...
**Prerequisites:** Docker + Docker Compose, Node 20+, Python 3.12+ (AI services), optionally AWS CLI.
...
```bash
chmod +x all.sh && ./all.sh     # then log out/in (or `newgrp docker`)

```bash
cp .env.example .env                # fill in what you have; everything runs with defaults
docker compose up --build -d        # postgres, redis, mailhog, minio, prometheus, grafana, all services
```
...
**Seeded demo logins** (from `backend-core/prisma/seed.ts`):
...
terraform init                       # remote state: S3 + DynamoDB locks
terraform plan
terraform apply
```
...
- **Add an alert rule:** append to `monitoring/prometheus/alerts.yml` (expression, duration, severity, runbook link), then route the severity in Alertmanager. Restart `docker compose restart prometheus alertmanager`.
